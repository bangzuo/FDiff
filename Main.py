	...
	def trainEpoch(self):
		...
		with torch.no_grad():
			...
			for _, batch in enumerate(diffusionLoader):
				...
				# MFF
				batch_f = Filter(denoised_batch)
				fs = batch_f.shape[0]
				t = np.arange(0, 1, 1 / fs)
				batch_f_1 = batch_f.cpu()
				batch_f_2 = batch_f_1.detach().numpy()
				# MFF cutoff frequency in MIND Dataset
				fch = fs / 2 - fs / 10
				fcl = fs / 10
				bandwidth = fch - fcl
				rp = 5
				order = 5
				b1, a1 = signal.cheby2(order, rp, fch, fs=fs, btype='high')
				b11, a11 = signal.cheby1(order, rp, fcl, fs=fs, btype='low')
				fch_butter = fch / fs
				fcl_butter = fcl / fs
				bp, ap = signal.butter(order, [fcl_butter, fch_butter], btype='band')
				filtered_x1 = signal.lfilter(b1, a1, batch_f_2)
				filtered_x11 = signal.lfilter(b11, a11, batch_f_2)
				filtered_xp1 = signal.lfilter(bp, ap, batch_f_2)
				# weights of MFF in MIND Dataset
				filtered_all = 0.2 * filtered_x1 + 0.2 * filtered_x11 + 0.6 * filtered_xp1
				filtered_all = np.float32(filtered_all)
				filtered_all = torch.tensor(filtered_all)
				filtered_all = filtered_all.cuda()
				denoised_batch = IFilter(filtered_all)
